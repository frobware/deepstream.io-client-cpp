...
State transition(State state, const Message &message, Sender sender) {
  ...
  if (state == State::AWAIT_CONNECTION && topic == Topic::CONNECTION &&
      action == Action::CHALLENGE && sender == Sender::SERVER) {
    assert(!is_ack);

    return State::CHALLENGING;
  }

  if (state == State::CHALLENGING && topic == Topic::CONNECTION &&
      action == Action::CHALLENGE_RESPONSE && !is_ack &&
      sender == Sender::CLIENT) {
    return State::CHALLENGING_WAIT;
  }

  if (state == State::CHALLENGING_WAIT && topic == Topic::CONNECTION &&
      action == Action::CHALLENGE_RESPONSE && is_ack &&
      sender == Sender::SERVER) {
  ...
  if (state == State::CHALLENGING_WAIT && topic == Topic::CONNECTION &&
      action == Action::REDIRECT && sender == Sender::SERVER) {
    assert(!is_ack);
  ...
  if (state == State::CHALLENGING_WAIT && topic == Topic::CONNECTION &&
      action == Action::REJECT && sender == Sender::SERVER) {
    assert(!is_ack);

    return State::DISCONNECTED;
  }

  if (state == State::AWAIT_AUTHENTICATION && topic == Topic::AUTH &&
      action == Action::REQUEST && !is_ack && sender == Sender::CLIENT) {
    return State::AUTHENTICATING;
  }

  if (state == State::AUTHENTICATING && topic == Topic::AUTH &&
      action == Action::REQUEST && is_ack && sender == Sender::SERVER) {
  ...
  if (state == State::AUTHENTICATING && topic == Topic::AUTH &&
      action == Action::ERROR_INVALID_AUTH_DATA && sender == Sender::SERVER) {
    assert(!is_ack);
  ...
  if (state == State::AUTHENTICATING && topic == Topic::AUTH &&
      action == Action::ERROR_INVALID_AUTH_MSG && sender == Sender::SERVER) {
    assert(!is_ack);
  ...
  if (state == State::CONNECTED && topic == Topic::EVENT) {
    if (action == Action::LISTEN || action == Action::SUBSCRIBE ||
        action == Action::UNLISTEN || action == Action::UNSUBSCRIBE ||
        action == Action::LISTEN_ACCEPT || action == Action::LISTEN_REJECT) {
