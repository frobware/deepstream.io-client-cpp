...
BOOST_AUTO_TEST_CASE(simple) {
  std::unique_ptr<Client> p =
      Client::make(std::unique_ptr<websockets::Client>(new SimpleClient),
...
struct RedirectionClient : public websockets::pseudo::Client {
  ...
  RedirectionClient(const std::string &uri = DEFAULT_URI,
      ...
      : state_(client::State::AWAIT_CONNECTION), do_redirect_(do_redirect),
        uri_(uri) {}
...
BOOST_AUTO_TEST_CASE(redirections) {
  std::unique_ptr<Client> p =
      Client::make(std::unique_ptr<websockets::Client>(new RedirectionClient),
                   std::unique_ptr<ErrorHandler>(new FailHandler));
